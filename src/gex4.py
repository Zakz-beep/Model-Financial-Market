...
def compute_all_greeks_for_bucket(df: pd.DataFrame, spot: float, r: float = 0.05):
    ...
    # ══════════════════════════════════════════
    #  DELTA-ADJUSTED OI (DAI)
    #
    #  DAI = |Delta| × OI × 100  per kontrak
    #
    #  Mengapa |Delta| bukan Delta signed?
    #  Karena kita ukur HEDGING PRESSURE — dealer
    #  harus hedge terlepas dari arah posisi.
    #  Call delta [0,1], Put delta [-1,0] → abs.
    #
    #  Call DAI = +|delta| × OI × 100  (beli underlying saat hedge)
    #  Put  DAI = -|delta| × OI × 100  (jual underlying saat hedge)
    #  Net  DAI = Call DAI + Put DAI    (net directional hedging pressure)
    #
    #  Interpretasi:
    #  - Net DAI positif di strike X → dealer net long delta disana
    #    → cenderung jual underlying kalau spot naik ke X (resistance)
    #  - Net DAI negatif → dealer net short delta → beli saat spot turun (support)
    #  - |Call DAI| vs |Put DAI| → lihat mana yang dominan per strike
    # ══════════════════════════════════════════
    df["dai_raw"] = np.abs(df["delta"]) * df["openInterest"] * mult

    # Call DAI: positif (dealer long delta → jual saat hedge)
    # Put  DAI: negatif (dealer short delta → beli saat hedge)
    df["dai_signed"] = np.where(
        df["tipe"] == "Call",
        +np.abs(df["delta"]) * df["openInterest"] * mult,
        -np.abs(df["delta"]) * df["openInterest"] * mult,
    )

    # Agregasi terpisah per tipe dan net
    call_dai_df = (
        df[df["tipe"] == "Call"]
        .groupby("strike", as_index=False)["dai_raw"]
        .sum()
        .rename(columns={"dai_raw": "call_dai"})
    ...
    put_dai_df = (
        ...
        .groupby("strike", as_index=False)["dai_raw"]
        .sum()
        .rename(columns={"dai_raw": "put_dai"})
        .sort_values("strike").reset_index(drop=True)
    )
    net_dai_df = (
        df.groupby("strike", as_index=False)["dai_signed"]
        .sum()
        .rename(columns={"dai_signed": "net_dai"})
        .sort_values("strike").reset_index(drop=True)
    )

    # Merge semua ke satu df
    dai_df = net_dai_df.merge(call_dai_df, on="strike", how="left") \
                       .merge(put_dai_df,  on="strike", how="left") \
                       .fillna(0)

    # Strike dengan net DAI terbesar (most hedged strike)
    peak_dai_strike = None
    if not dai_df.empty:
        peak_dai_strike = float(dai_df.loc[dai_df["net_dai"].abs().idxmax(), "strike"])

    # DAI Skew per strike: call_dai / (call_dai + put_dai)
    # > 0.5 = call-heavy (bullish hedging), < 0.5 = put-heavy (bearish hedging)
    dai_df["dai_skew"] = dai_df["call_dai"] / (
        dai_df["call_dai"] + dai_df["put_dai"]
    ).replace(0, np.nan)
...
def build_html(bucket_data: dict, spot: float, ticker: str = "SPY") -> str:
    ...
    for bucket, info in bucket_data.items():
        ...
        def dai_to_list(col, scale=1e3):
            return [round(float(v) / scale, 3) for v in dai_df[col].tolist()]

        js_data[bucket] = {
            "gex":        df_to_lists(gex_df,  "net_gex",  1e6),
            "vex":        df_to_lists(vex_df,  "net_vex",  1e6),
            "chex":       df_to_lists(chex_df, "net_chex", 1e3),
            "vega":       df_to_lists(vega_df, "net_vega", 1e6),
            "dai": {
                "strikes": dai_df["strike"].tolist(),
                "net":     dai_to_list("net_dai"),
                "call":    dai_to_list("call_dai"),
                "put":     dai_to_list("put_dai"),
                "skew":    [round(float(v), 3) if not np.isnan(v) else 0.5
                            for v in dai_df["dai_skew"].tolist()],
            },
            "flip":        info["flip"],
...
.greek-btn.active.dai   {{ color: var(--dai);   border-color: var(--dai);   background: rgba(52,211,153,0.08); }}
...
.badge.pdai   {{ color: var(--dai);   border-color: rgba(52,211,153,0.35);  background: rgba(52,211,153,0.06); }}
...
<header>
  <div class="header-left">
    ...
    <p>GEX · Vanna · Charm · Vega · DAI Dashboard &bull; {today_str}</p>
  </div>
...
<div class="stats-row">
  ...
  <div class="stat-card"><div class="s-lbl">Peak DAI</div><div class="s-val dai" id="st-dai">—</div></div>
</div>
...
const GREEK_CONFIGS = {{
  ...
  dai: {{
    label: "DAI",
    key: "dai",   // special — punya renderer sendiri
    unit: "K",
    posColor: "#34d399",
    negColor: "#f87171",
    desc: "<strong>Delta-Adjusted OI (DAI)</strong> — OI × |Delta| per strike. Berbeda dari raw OI yang tidak memperhitungkan moneyness. DAI mengukur <strong>actual hedging pressure</strong> dealer di tiap strike level. <br><br>• <span style='color:#34d399'>Call DAI</span> = berapa delta yang harus dealer hedge dari posisi call mereka (→ jual underlying saat spot naik ke sini) <br>• <span style='color:#f87171'>Put DAI</span> = berapa delta yang harus dealer hedge dari posisi put mereka (→ beli underlying saat spot turun ke sini) <br>• <strong>Net DAI</strong> = selisih keduanya → directional bias hedging. Net positif = resistance, net negatif = support. <br>• <strong>DAI Skew bar</strong> (kanan) = rasio call/(call+put) per strike. >50% = bullish bias, <50% = bearish bias.",
    wallKeys: [],
    peakKey: "peak_dai",
  }},
}};
...
function drawGreekChart(canvasId, bucket, greekKey) {{
  ...
  if (d.peak_dai)   specialStrikes[d.peak_dai.toFixed(1)]   = {{ type: "pdai",   color: "#34d399", label: "DAI↑"    }};
...
// ── DAI Chart (special grouped renderer) ──────
function drawDAIChart(canvasId, bucket) {{
  const canvas = document.getElementById(canvasId);
  if (!canvas) return;
  const d   = DATA[bucket];
  const dai = d.dai;
  if (!dai || dai.strikes.length === 0) return;

  const strikes  = dai.strikes;
  const netVals  = dai.net;
  const callVals = dai.call;
  const putVals  = dai.put;
  const skewVals = dai.skew;
  const n        = strikes.length;

  const rowH  = 26;         // sedikit lebih tinggi untuk grouped bar
  const padL  = 72;
  const padR  = 110;        // lebih lebar untuk skew bar di kanan
  const padT  = 20;
  const padB  = 32;
  const W     = canvas.parentElement.clientWidth || 960;
  ...
  // Split chart area: left 75% = bar chart, right 25% = skew bar
  const barAreaW = (W - padL - padR) * 0.75;
  const skewAreaX = padL + barAreaW + 12;
  const skewAreaW = W - skewAreaX - padR + 80;
  const zeroX    = padL + barAreaW / 2;
  ...
  // Special strike marks
  const specialStrikes = {{}};
  if (d.flip)      specialStrikes[d.flip.toFixed(1)]      = {{ color: "#ffd166", label: "FLIP"   }};
  if (d.call_wall) specialStrikes[d.call_wall.toFixed(1)] = {{ color: "#00c896", label: "C.WALL" }};
  if (d.put_wall)  specialStrikes[d.put_wall.toFixed(1)]  = {{ color: "#ff4060", label: "P.WALL" }};
  if (d.peak_dai)  specialStrikes[d.peak_dai.toFixed(1)]  = {{ color: "#34d399", label: "DAI↑"   }};

  // Section divider
  ctx.strokeStyle = "#1a2535"; ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(skewAreaX - 6, padT);
  ctx.lineTo(skewAreaX - 6, H - padB);
  ctx.stroke();

  // Skew header
  ctx.fillStyle = "#3a4d65"; ctx.font = "7px 'Space Mono', monospace"; ctx.textAlign = "center";
  ctx.fillText("SKEW", skewAreaX + skewAreaW / 2, padT - 4);
  ctx.fillStyle = "#34d399"; ctx.fillText("CALL", skewAreaX + skewAreaW * 0.15, padT - 4);
  ctx.fillStyle = "#f87171"; ctx.fillText("PUT",  skewAreaX + skewAreaW * 0.85, padT - 4);

  for (let i = 0; i < n; i++) {{
    const y       = padT + i * rowH;
    const barH    = rowH - 6;
    const callLen = (callVals[i] / maxAbs) * (barAreaW / 2);
    const putLen  = (putVals[i]  / maxAbs) * (barAreaW / 2);
    const net     = netVals[i];
    const skew    = skewVals[i];   // 0..1

    const isSpot = Math.abs(strikes[i] - SPOT) < 0.51;
    const sKey   = strikes[i].toFixed(1);
    const sp     = specialStrikes[sKey];

    // ── Call bar (right of zero, teal) ──
    ctx.fillStyle = "rgba(52,211,153,0.15)";
    ctx.fillRect(zeroX, y + 2, callLen, barH);
    ctx.fillStyle = "#34d399";
    ctx.fillRect(zeroX, y + 2, callLen, 2);
    ctx.fillRect(zeroX, y + 2, Math.min(callLen, 2), barH);

    // ── Put bar (left of zero, red) ──
    ctx.fillStyle = "rgba(248,113,113,0.15)";
    ctx.fillRect(zeroX - putLen, y + 2, putLen, barH);
    ctx.fillStyle = "#f87171";
    ctx.fillRect(zeroX - putLen, y + 2, putLen, 2);
    ctx.fillRect(zeroX + putLen - Math.min(putLen, 2), y + 2, Math.min(putLen, 2), barH);

    // ── Net marker (thin line on top of call/put area) ──
    const netX = net >= 0 ? zeroX + (net / maxAbs) * (barAreaW / 2) : zeroX - (Math.abs(net) / maxAbs) * (barAreaW / 2);
    ctx.fillStyle = net >= 0 ? "#34d399" : "#f87171";
    ctx.fillRect(netX - 1, y + 2, 2, barH);
    ...
    if (callLen > 18) {{
      ctx.fillStyle = "#34d399"; ctx.font = "7px 'Space Mono', monospace"; ctx.textAlign = "left";
      ctx.fillText("+" + callVals[i].toFixed(1) + "K", zeroX + callLen + 2, y + rowH / 2 + 2);
    }}
    if (putLen > 18) {{
      ctx.fillStyle = "#f87171"; ctx.font = "7px 'Space Mono', monospace"; ctx.textAlign = "right";
      ctx.fillText("-" + putVals[i].toFixed(1) + "K",  zeroX - putLen - 2, y + rowH / 2 + 2);
    }}

    // ── Skew bar (right section) ──
    // Background
    ctx.fillStyle = "#141c28";
    ctx.fillRect(skewAreaX, y + 3, skewAreaW, barH - 2);
    // Call portion (left = green)
    const callSkewW = skewAreaW * Math.min(Math.max(skew, 0), 1);
    ctx.fillStyle = "rgba(52,211,153,0.35)";
    ctx.fillRect(skewAreaX, y + 3, callSkewW, barH - 2);
    // Put portion (right = red)
    ctx.fillStyle = "rgba(248,113,113,0.35)";
    ctx.fillRect(skewAreaX + callSkewW, y + 3, skewAreaW - callSkewW, barH - 2);
    // 50% line
    ctx.strokeStyle = "#2a3a52"; ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(skewAreaX + skewAreaW / 2, y + 3);
    ctx.lineTo(skewAreaX + skewAreaW / 2, y + barH + 1);
    ctx.stroke();
    // Skew % text
    ctx.fillStyle = skew > 0.5 ? "#34d399" : "#f87171";
    ctx.font = "7.5px 'Space Mono', monospace"; ctx.textAlign = "center";
    ctx.fillText((skew * 100).toFixed(0) + "%", skewAreaX + skewAreaW / 2, y + rowH / 2 + 3);

    // ── Spot / special lines ──
    if (isSpot) {{
      ctx.strokeStyle = "rgba(255,209,102,0.5)"; ctx.lineWidth = 1; ctx.setLineDash([4,3]);
      ctx.beginPath(); ctx.moveTo(padL, y + rowH/2); ctx.lineTo(skewAreaX + skewAreaW, y + rowH/2); ctx.stroke();
      ctx.setLineDash([]);
    }}
    if (sp && !isSpot) {{
      ctx.strokeStyle = sp.color + "55"; ctx.lineWidth = 1; ctx.setLineDash([3,3]);
      ctx.beginPath(); ctx.moveTo(padL, y + rowH/2); ctx.lineTo(zeroX + barAreaW/2 + 2, y + rowH/2); ctx.stroke();
      ctx.setLineDash([]);
      ctx.fillStyle = sp.color; ctx.font = "bold 7px 'Space Mono', monospace"; ctx.textAlign = "right";
      ctx.fillText(sp.label, skewAreaX - 10, y + rowH/2 - 3);
    }}
  }}

  // ── Zero line ──
  ctx.strokeStyle = "#2a3545"; ctx.lineWidth = 1.5;
  ctx.beginPath(); ctx.moveTo(zeroX, 0); ctx.lineTo(zeroX, H - padB); ctx.stroke();

  // ── X axis ──
  const steps = [-1, -0.5, 0, 0.5, 1];
  ctx.fillStyle = "#3a4d65"; ctx.font = "7.5px 'Space Mono', monospace"; ctx.textAlign = "center";
  for (const s of steps) {{
    const px = zeroX + s * (barAreaW / 2);
    ctx.fillText((s * maxAbs).toFixed(1) + "K", px, H - 8);
    ctx.strokeStyle = "#141e2e"; ctx.lineWidth = 1; ctx.setLineDash([3,4]);
    ctx.beginPath(); ctx.moveTo(px, padT); ctx.lineTo(px, H - padB); ctx.stroke();
    ctx.setLineDash([]);
  }}
  ...
  ctx.fillStyle = "#34d399"; ctx.fillText("■ Call DAI", padL, padT - 5);
  ctx.fillStyle = "#f87171"; ctx.fillText("■ Put DAI",  padL + 72, padT - 5);
  ctx.fillStyle = "#aaa";    ctx.fillText("│ Net DAI",  padL + 144, padT - 5);
}}
...
function buildPanels() {{
  ...
  for (let i = 0; i < BUCKETS.length; i++) {{
    ...
    if (d.peak_dai)  badgesHtml += `<span class="badge pdai">Δ DAI ${{d.peak_dai.toFixed(2)}}</span>`;
...
function switchGreek(bucket, greek) {{
  ...
  // DAI pakai renderer khusus
  if (greek === "dai") {{
    setTimeout(() => drawDAIChart(`canvas-${{bucket}}-dai`, bucket), 40);
  }} else {{
...
function updateStats(bucket) {{
  ...
  document.getElementById("st-dai").textContent    = d.peak_dai   ? "$" + d.peak_dai.toFixed(2)   : "N/A";
}}
...
function switchBucket(bucket) {{
  ...
  if (gk === "dai") {{
    setTimeout(() => drawDAIChart(`canvas-${{bucket}}-dai`, bucket), 50);
  }} else {{
...
if __name__ == "__main__":
    TICKER         = "SPY"
    RISK_FREE_RATE = 0.05
    OUTPUT_HTML    = "SPY_Greeks_dashboard.html"
    ...
    print("\n[GREEKS] Menghitung Full Greeks + DAI per expiry bucket...")
    for bucket in BUCKET_ORDER:
        ...
        print(f"  [{bucket}] {len(result['gex_df'])} strikes | "
              ...
              f"PeakDAI: {'$'+str(round(result['peak_dai'],2)) if result['peak_dai'] else 'N/A'}")
    ...
    print(f"       Buka di browser untuk melihat GEX, Vanna, Charm, Vega, DAI per expiry bucket.")